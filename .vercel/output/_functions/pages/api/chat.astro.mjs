...
setInterval(() => {
  ...
  for (const [ip, record] of rateLimitMap.entries()) {
    if (now - record.dayTimestamp > 864e5) {
...
function checkRateLimit(ip) {
  ...
  if (now - record.minuteTimestamp > 6e4) {
  ...
  if (record.minuteCount >= 5) {
    const retryAfter = Math.ceil((6e4 - (now - record.minuteTimestamp)) / 1e3);
    return {
      allowed: false,
      retryAfter,
...
const POST = async ({ request }) => {
  try {
    ...
    const formattedHistory = Array.isArray(history) ? history.slice(-4).map((msg) => ({
    ...
    const apiKey = undefined                                  || process.env.FIREWORKS_API_KEY;
